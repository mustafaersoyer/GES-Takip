...
// aldığımız işlem değişkenine göre işlemler yapalım.
  if($_method  == "POST") {
    ...
    if(isset($_GET["tesisId"])) {

       $tesisId= ($_GET["tesisId"]);
       //if(tarih: yıl ise aylik uretim çalıştır)
      //else (tarih: yıl-ay-gun ise saatlik uretim çalıştır)
       $bilgiler = $db->query("SELECT * FROM arizalicihazlar where tesisId like tesisId='$tesisId'", PDO::FETCH_ASSOC);
       ...
       $_code = 400;
       $jsonArray["hata"] = TRUE; // bir hata olduğu bildirilsin.
       $jsonArray["hataMesaj"] = "Lütfen tesisId değişkeni gönderin"; // Hatanın neden kaynaklı olduğu belirtilsin.
    }
